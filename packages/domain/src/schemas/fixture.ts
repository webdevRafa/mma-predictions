...
const idSchema = z
  ...
  .regex(
    /^[a-z]{3}_[a-z0-9_]+$/,
    "Use a stable prefixed ID such as evt_demo_001",
  );
const slugSchema = z
  .string()
  .regex(
...
export const fixtureFighterSchema = z
  .object({
    id: idSchema,
    slug: slugSchema,
  ...
  .superRefine((summary, context) => {
    if (summary.fighterA + summary.fighterB !== summary.total) {
      context.addIssue({
        code: "custom",
        path: ["total"],
        message: "total must equal fighterA + fighterB",
      });
    }
  });
...
export const fixtureFightSchema = z
  .object({
    id: idSchema,
    slug: slugSchema,
    slugHistory: z.array(slugSchema).default([]),
...
export const normalizedFixtureSchema = z
  ...
  .superRefine((fixture, context) => {
    ...
    fixture.fighters.forEach((fighter, index) => {
      if (fighterIds.has(fighter.id))
        context.addIssue({
          code: "custom",
          path: ["fighters", index, "id"],
          message: "Duplicate fighter ID",
        });
    ...
    fixture.fights.forEach((fight, index) => {
      if (fightIds.has(fight.id))
        context.addIssue({
          code: "custom",
          path: ["fights", index, "id"],
          message: "Duplicate fight ID",
        });
      fightIds.add(fight.id);
      if (fight.eventId !== fixture.event.id)
        context.addIssue({
          code: "custom",
          path: ["fights", index, "eventId"],
          message: "Fight eventId must match fixture event ID",
        });
      if (!fighterIds.has(fight.fighterAId))
        context.addIssue({
          code: "custom",
          path: ["fights", index, "fighterAId"],
          message: "Unknown fighter ID",
        });
      if (!fighterIds.has(fight.fighterBId))
        context.addIssue({
          code: "custom",
          path: ["fights", index, "fighterBId"],
          message: "Unknown fighter ID",
        });
      if (fight.fighterAId === fight.fighterBId)
        context.addIssue({
          code: "custom",
          path: ["fights", index, "fighterBId"],
          message: "A fighter cannot face themselves",
        });
      if (fight.isTitleFight && fight.scheduledRounds !== 5)
        context.addIssue({
          code: "custom",
          path: ["fights", index, "scheduledRounds"],
          message: "Title fights must be scheduled for five rounds",
        });
      if (fight.status === "completed" && !fight.result)
        context.addIssue({
          code: "custom",
          path: ["fights", index, "result"],
          message: "Completed fights require a result",
        });
      if (
        fight.result?.winnerFighterId &&
        ![fight.fighterAId, fight.fighterBId].includes(
          fight.result.winnerFighterId,
        )
      )
        context.addIssue({
          code: "custom",
          path: ["fights", index, "result", "winnerFighterId"],
          message: "Winner must be a fighter in the matchup",
        });
    });
    ...
    fixture.fights.forEach((fight, index) => {
      if (fight.replacedByFightId && !fightIds.has(fight.replacedByFightId))
        context.addIssue({
          code: "custom",
          path: ["fights", index, "replacedByFightId"],
          message: "Replacement fight ID does not exist",
        });
    });
  });
