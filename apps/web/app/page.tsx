...
const benefits = [
  {
    icon: Target,
    label: "Make the call",
    copy: "Pick the winner, method, and finish detail before walkouts.",
  },
  {
    icon: MessageCircle,
    label: "Join the room",
    copy: "Every matchup has a focused lobby for live reactions.",
  },
  {
    icon: Trophy,
    label: "Keep receipts",
    copy: "Build an honest record of points, accuracy, and streaks.",
  },
];
    ...
    <main id="main-content">
      <section className="relative overflow-hidden border-b border-fl-border">
        ...
        <div className="shell relative grid min-h-[calc(100vh-4.5rem)] items-center gap-12 py-16 lg:grid-cols-[1.04fr_.96fr] lg:py-24">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-6 font-display text-[clamp(4rem,10vw,8.8rem)] leading-[0.78] font-extrabold tracking-[-0.035em] text-balance">
              EVERY FIGHT
            ...
            <p className="mt-7 max-w-xl text-base leading-7 text-fl-text-muted sm:text-lg">
              Make your pick before the walkout, reveal the community read, and
              own the record afterward.
            </p>
          ...
          <Card className="relative overflow-hidden lg:ml-auto lg:w-full lg:max-w-xl">
            ...
            <div className="space-y-4 p-5">
              <div className="rounded-xl border border-fl-border bg-fl-surface-2 p-4">
                <div className="flex items-center gap-2 font-mono text-[10px] tracking-[0.08em] text-fl-text-muted uppercase">
                  <CalendarDays aria-hidden="true" size={14} /> Fixture provider
                  connecting next
                </div>
                <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-end gap-4">
                  ...
                  <span className="font-display text-xl font-extrabold text-fl-text-dim">
                    VS
                  </span>
                  <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {["Fight card", "Community", "Lobby"].map((label) => (
                  <div
                    className="rounded-lg border border-fl-border bg-fl-surface-2 px-3 py-3 text-center text-[11px] font-semibold text-fl-text-muted"
                    key={label}
                  >
                    {label}
                  </div>
      ...
      <section className="shell py-16 sm:py-20">
        <div className="grid gap-px overflow-hidden rounded-2xl border border-fl-border bg-fl-border md:grid-cols-3">
          {benefits.map(({ copy, icon: Icon, label }) => (
            <article className="bg-fl-surface-1 p-6 sm:p-8" key={label}>
              <Icon aria-hidden="true" className="text-fl-accent" size={22} />
